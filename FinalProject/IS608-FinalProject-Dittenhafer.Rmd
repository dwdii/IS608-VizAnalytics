...
{
  # Load the Natality data
  dataFile <- "C:/Code/R/IS608-VizAnalytics/FinalProject/Data/CO-EST2014-Alldata.csv"
...
birthFile <- "C:/Code/R/IS608-VizAnalytics/FinalProject/Data/Natality-2007-2013-YearMonthStateAge.txt"
...
if(doBirthRateCombine)
{
  ...
  udbcAge <- subset(udbc, 
                       select=c("Age.of.Mother",
                                ...
                                "UnemploymentRate",
                                "CENSUS2010POPThousands"))  
  udbcAge <- aggregate(Births ~ Age.of.Mother+Year.Code+Month.Code+UnemploymentRate+CENSUS2010POPThousands, udbcAge, sum)
  ...
  if(bSave)
  {
    dataFile <- sprintf("%s/LA-Natality-Census-Combined.csv", path)
    write.table(udbcState, dataFile,sep=",", row.names=FALSE)  
         
    dataFile <- sprintf("%s/LA-Natality-Census-Age-Combined.csv", path)
...
g11 <- g1 + geom_point(data=unempData, aes(y=-5255729 + 
...
```{r annual_births, fig.path='figure/'}

yrTotals <- aggregate(Births~ Year, udbc, FUN=sum )
print(yrTotals)

gT <- ggplot(yrTotals, aes(x=Year)) + geom_line(aes(y=Births)) +
      labs(title="United States Births 2003 - 2013 Annually",
                x="Time",
                y="Births") +
